(function () {
  ...
  function getSelectedQty(productId) {
    ...
    const qtyInput =
      document.querySelector(`[data-qty-for="${productId}"]`);
  ...
  function addToCart(product) {
    ...
    const maxStock =
      product.size
        ? availableStock(product.id, product.size)
        : Infinity;
  ...
  function markSizeField(select, hasError) {
    ...
    if (hasError) {
      ...
      if (wrapLabel) {
        wrapLabel.classList.add('size-field--error');
      }
      ...
      if (wrapLabel) {
        wrapLabel.classList.remove('size-field--error');
      }
  ...
  document.querySelectorAll('[data-add]').forEach(btn => {
    ...
    btn.addEventListener('click', () => {
      ...
      const requiresSize =
        btn.dataset.requiresSize === 'true';

      const sizeSelect =
        document.querySelector(
          `[data-size-for="${btn.dataset.id}"]`
        );
      ...
      const qty =
        getSelectedQty(btn.dataset.id);


      if (requiresSize && !size) {

        alert(
          'Select a size first: M, L, XL or XXL.'
        );
      ...
      if (sizeSelect) {
        markSizeField(sizeSelect, false);
      }
      ...
      itemsWrap.innerHTML = `
        ...
        <article class="card">
          ...
          <p class="empty-state">
            ...
            Your cart is empty.
            Go back to the shop and add the records
            or merch you want to order.
          ...
          const cart =
            readCart()
              ...
              .map(item =>
                ...
                (
                  item.id === btn.dataset.minus &&
                  String(item.size || '') ===
                  String(btn.dataset.size || '')
                )
...
document.querySelectorAll('[data-player]').forEach(player => {
  ...
  const audio =
    player.querySelector('[data-audio]');

  const playBtn =
    player.querySelector('[data-play]');

  const progress =
    player.querySelector('[data-progress]');

  const time =
    player.querySelector('[data-time]');

  const title =
    player.querySelector('[data-track-title]');

  const tabs =
    player.querySelectorAll('[data-track]');
  ...
  playBtn?.addEventListener('click', () => {
    ...
    if (audio.paused) {
      ...
      audio.play().catch(() => {
        ...
        alert(
          'Add the preview files in /audio to activate the custom player.'
        );
...
(function () {
  ...
  const requestSummary =
    document.getElementById('request-summary');
  ...
  const raw =
    sessionStorage.getItem('circuit_request');
  ...
  try {
    ...
    const sizeInput =
      document.getElementById('order-size');
